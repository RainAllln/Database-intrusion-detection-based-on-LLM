...
def main():
    ...
    # 使用 AST 展平
    df['clean_query'] = df['query'].apply(preprocessor.normalize_and_flatten)
    ...
    print(f"特征提取完成。DistilBERT 维数: {embeddings.shape[1]}")

    # 4. 划分训练集
    X_train, X_test, y_train, y_test = train_test_split(X_combined, y_for_layer1, test_size=0.2, random_state=42)

    # 第一层训练策略：仅使用 Label 为 0 和 2 的样本进行训练
    X_train_normal = X_train[y_train == 0]

    # --- 创建实验文件夹 ---
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model = "lof"
    camel_case_model = to_camel_case(model)
    exp_folder = f"exp1_{data_set}_{camel_case_model}_{timestamp}"
    ...
    try:
        # scores 一般越大越“正常”，为便于 roc_auc，取负号使“越大越攻击”
    ...
    report_note = (
        ...
        f"Training Samples Used: {len(X_train_normal)}\n"
        f"Test Samples: {len(X_test)}\n\n"
    )
    report_params = (
        f"preprocessor: Normalize & Flatten (AST)\n"
        f"feature: {extractor_type}\n"
        f"Model: {model}\n"
        f"Hyperparameters:\n{l1_detector.get_hyperparams_str()}\n"
        "Feature Type: Flattened AST Sequence\n"
    )
    report_content = (
        "--- Evaluation Metrics ---\n"
        f"Accuracy: {acc:.4f}\n"
    ...
    norm_new = preprocessor.normalize_and_flatten(new_query)
    new_bert_emb = embedder.get_embeddings([norm_new])

    new_emb = new_bert_emb
